...
- Public posts and OAuth/MCP state belong to the `BLOG_DB` D1 binding. Durable OAuth code/token tables live in this same database, as does the `oauth_clients` table that stores RFC 7591 dynamically registered clients. Client ID Metadata Document (CIMD) validation is implemented per authorization request in MoonBit; there is no metadata cache.
...
- `oauth_clients` stores dynamically registered clients (RFC 7591): a unique opaque `client_id`, `client_name`, the declared `redirect_uris` as a JSON array, an optional `scope` restriction, a nullable `client_secret_hash` (SHA-256 hex only, never the presented secret), `created_at`, and nullable `revoked_at`.
- No plaintext code, token, or client-secret column exists. All repository SQL uses bound parameters.

## Authorization slice

- `client_id` is either a CIMD metadata document URL or an RFC 7591 registered identifier. For a CIMD URL, `GET /oauth/authorize` fetches the document through a narrowly-scoped JS `fetch` capability (HTTPS only, no redirects, anonymous, timeout, size and content-type limits) and validates it in pure MoonBit: the document `client_id` must equal the requested URL, at least one `redirect_uris` entry must be declared, and every declared URI must be absolute HTTPS without a fragment. For any other `client_id` shape, the authorize endpoint looks up a live (non-revoked) `oauth_clients` row and rebuilds the same `ClientMetadata` value from its stored, re-validated `redirect_uris`.
- The authorization request binds `response_type=code`, the resolved client, an exactly matching declared `redirect_uri`, the exact `SITE_URL + /mcp` resource, validated space-delimited scopes, mandatory PKCE `S256`, and preserved `state`. A registered client's stored `scope` (when non-empty) additionally bounds the requested scopes; a violation redirects with `invalid_scope`.
...
- `POST /oauth/token` accepts only `application/x-www-form-urlencoded` and rejects malformed encoding, duplicate security fields, unknown fields, and unsupported grants. The supported grants are `authorization_code` and `refresh_token` only. An optional `client_secret` field and HTTP Basic credentials are accepted for confidential clients.
- When the request's `client_id` resolves to a registered client with a stored `client_secret_hash`, the presented secret — from the `client_secret` form field or a matching Basic credential — must hash to that value, or the request fails with `invalid_client`. CIMD clients and public (`token_endpoint_auth_method: "none"`) registrations have no secret to verify.

## Dynamic client registration slice

- `POST /oauth/register` implements RFC 7591 for clients that cannot use CIMD (for example the ChatGPT connector, which requires a pre-registered client). The endpoint accepts a JSON body with mandatory `redirect_uris` (at most 10, each an absolute HTTPS URI without a fragment), optional `client_name`, `token_endpoint_auth_method` (`none`, `client_secret_post`, or `client_secret_basic`; default `none`), optional `grant_types` (subset of `authorization_code`/`refresh_token` containing `authorization_code`), optional `response_types` (`["code"]` only), and an optional `scope` that must be a subset of the supported scopes.
- Registration issues a `dcr-`-prefixed opaque `client_id`. Confidential auth methods mint a 256-bit `client_secret` returned exactly once; only its SHA-256 hash is stored. Public clients receive no secret and rely on PKCE alone.
- The response is the RFC 7591 client information document (`client_id`, `client_id_issued_at`, optional `client_secret` with `client_secret_expires_at: 0`, `client_name`, `redirect_uris`, `grant_types`, `response_types`, `token_endpoint_auth_method`, and `scope` when restricted) with `201` and `no-store` headers. Invalid metadata fails with `{"error":"invalid_client_metadata"}` and HTTP 400.
- The authorization-server metadata advertises `registration_endpoint` and `token_endpoint_auth_methods_supported` so compliant clients discover DCR automatically.
...
- Every D1 statement reached from a Worker handler runs through a raw-JS `extern` adapter. The typed `@cloudflare` wrapper (`prepare().first()/.run()/.batch()`) suspends on `@js_async.Promise::wait`, which unwraps the moonbitlang/async coroutine scheduler's current task — `None` under the Worker's fetch driver — and panics. This previously broke the entire refresh grant in production; the remaining typed helpers in `oauth_repo` are not reachable from request handlers and would need the same conversion before use.
...
3. Route GET `/.well-known/oauth-protected-resource` and GET `/.well-known/oauth-authorization-server` before the D1 availability check. The metadata uses the configured canonical origin, exact issuer/resource values, PKCE S256, CIMD support, and the DCR `registration_endpoint`.
...
CIMD remains the preferred client-registration path. DCR is implemented as a narrowly scoped compatibility fallback for clients that cannot consume CIMD (the ChatGPT connector requires a pre-registered client): strict redirect-URI and client-metadata validation, HTTPS-only redirects, bounded grant/response types, optional scope restriction, and hashed-secret storage.
...
- Bearer protected-resource authorization middleware primitives are implemented: Authorization-header-only parsing, hash-only exact-resource validation, expiry/revocation and stored-scope integrity checks, distinct 401/403 decisions, and RFC 6750/RFC 9728 challenges. Required resource scopes are `mcp:discover`, `blog:read`, `blog:write`, and `blog:publish`; `offline_access` is not a resource permission. `mcp:discover` is intentionally separate from `blog:read`: authenticated protocol discovery exposes server identity/capability metadata, not blog content, and therefore should not grant content-read authority.
...
- RFC 7591 dynamic registration is implemented at `POST /oauth/register` with the `oauth_clients` D1 table (`migrate_oauth_v03.sql`).
- Add a short-lived CIMD metadata cache.
- Bearer protected-resource middleware primitives with exact resource and scope enforcement are implemented.
- Stateless MCP 2026-07-28 protocol primitives, the `tools/list` catalog, and `tools/call` execution are implemented in `src/mcp` with D1 access in the Worker bridge; see `docs/mcp-protocol-architecture.md`. `tools/list` is discovery-only and uses `mcp:discover`, with no new OAuth scope and no `blog:read` requirement. The catalog is exactly `list_posts`, `get_post`, `create_post`, `update_post`, `publish_post` in stable order, using narrow JSON Schema 2020-12 objects and integer post ids. `tools/call` resolves `params` to a typed plan first (`-32602` on schema violations), then the Worker enforces the per-tool scope: `blog:read` for list/get, `blog:write` for create/update, and `blog:publish` for publish; the `mcp:discover` gate still applies to every `/mcp` request first. `server/discover` advertises `capabilities.tools.listChanged: false`. Execution failures (not found, validation, storage) are CallToolResult `isError: true`, never JSON-RPC errors. Client self-reported info/capabilities never changes catalog or authz. A supplied tools/list cursor is invalid params (`-32602`) because this catalog has no pagination.
